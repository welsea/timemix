...
export default function EditDetail(props) {
  ...
  let [state, dispatch] = useReducer((state, action) => {
    switch (action.type) {
      ...
      case "TYPE":
        ...
        return { ...state };
      case "INFO":
        setContent({ ...content, info: action.value });
        return { ...state };
      default:
        return { ...state };
    }
  ...
  function startChange(e) {
    ...
    if (
      stype === 0 ||
      parseInt(time[0]) > parseInt(content.endPick.split(":")[0])
    ) {
      let tmpep = "";
      if (time[1] === "45") {
      ...
      <table className={ed.table}>
        <tbody>
          ...
          <tr>
            ...
            <td>
              <textarea
                defaultValue={content.info}
                onKeyUp={(e) =>
                  dispatch({ type: "INFO", value: e.target.value })
                }
              ></textarea>
            </td>
          </tr>
